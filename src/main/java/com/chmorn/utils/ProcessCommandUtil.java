...
public class ProcessCommandUtil {
	private static Logger logger = LoggerFactory.getLogger(ProcessCommandUtil.class);
	...
	public void start() {
		logger.info("执行命令开始：{}",this.command);
		...
		try {
			...
			logger.info("执行命令结束：耗时{}秒：" ,(end - start)/1000);
		} catch (Exception e) {
			logger.info("执行命令异常：{}",e.getMessage());
			processSuccess = false;
